    ...
    //    Vector<ContentValues> vectorOfCV = new Vector<ContentValues>();
        ...
        for (long movieId : mMovieIDs) {
            ...
            for (int index = 0; index < results_In_JSONArray.length(); index++) {
                ...
                contentValues.put(X_MovieReviewEntry.COL_REVIEWER, author);
                contentValues.put(X_MovieReviewEntry.COL_REVIEWCONTENT, content);
        ...
        if (vectorOfCV.size() > 0) {
            ...
            //-------------------
            //
            // context.getContentResolver().bulkInsert(X_MovieReviewEntry.CONTENT_URI, arrayOfCV);
            ...
            for (ContentValues cv : arrayOfCV) {
                ...
                //uri = X_MovieReviewEntry.buildUri_X_MovieReviewWithId(cv.getAsLong(X_MovieReviewEntry.COL_MV_ID));
                uri = X_MovieReviewEntry.CONTENT_URI;

                projection = new String[]{X_MovieReviewEntry._ID};

                selection = X_MovieReviewEntry.COL_MV_ID + "=? AND " +
                            X_MovieReviewEntry.COL_REVIEWER + "=?";

                selectionArg = new String[]{String.valueOf(cv.getAsLong(X_MovieReviewEntry.COL_MV_ID)),
                                            cv.getAsString(X_MovieReviewEntry.COL_REVIEWER)
                                            };
                ...
                if (cursor != null) {
                    if (!cursor.moveToFirst()) {
                        context.getContentResolver().insert(X_MovieReviewEntry.CONTENT_URI, cv);
                    }
                }
...
//        String movieInfoInJsonStr;// = null;
     ...
     //   Vector<ContentValues> vectorOfCV = new Vector<ContentValues>();
        Vector<ContentValues> vectorOfCV = new Vector<>();
            ...
            for (long movieId : mMovieIDs) {
                ...
                for (int i = 0; i < results_In_JSONArray.length(); i++) {
                    ...
                    if (i == 0) {
                        ...
                        contentValues.put(X_MovieVideosEntry.COL_VIDEO_KEY, aVideoKey);
                        contentValues.put(X_MovieVideosEntry.COL_MV_ID, movieId);

                        /*context.getContentResolver()
                                .insert(MovieContract.X_MovieVideosEntry.CONTENT_URI, contentValues);*/
                        vectorOfCV.add(contentValues);
                    }
                }
                //-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
            ...
            if (vectorOfCV.size() > 0) {
                ...
                //---------------------------------
                //context.getContentResolver().bulkInsert(X_MovieVideosEntry.CONTENT_URI, arrayOfCV);
                //-------------------
                Uri uri;
                String[] projection = null;
                String   selection ;
                ...
                for (ContentValues cv : arrayOfCV) {
                    ...
                    //uri = X_MovieVideosEntry.buildUri_X_MovieReviewWithId(cv.getAsLong(X_MovieVideosEntry.COL_MV_ID)); // ???
                    uri = X_MovieVideosEntry.CONTENT_URI;

                    projection = new String[]{X_MovieVideosEntry._ID};

                    selection = X_MovieVideosEntry.COL_MV_ID + "=?";

                    selectionArg = new String[]{String.valueOf(cv.getAsLong(X_MovieVideosEntry.COL_MV_ID))};

                    cursor = context.getContentResolver().query(uri, projection, selection, selectionArg, sortOrder);

                    if (cursor != null) {
                        if (!cursor.moveToFirst()) {
                            context.getContentResolver().insert(X_MovieVideosEntry.CONTENT_URI, cv);
                        }
                    }
    ...
    //=================================================
    //private
    public static Uri formUri_X_MovieInfo(Context mContext) {
    ...
    public static long[] get_X_MovieInfoFromJson(String moviesJsonStr,  String sortBy,  Context context) throws JSONException {
...
//        Vector<ContentValues> mVofCV = new Vector<ContentValues>(results_JSONArray.length());
        ...
        for (int i = 0; i < results_JSONArray.length(); i++) {
          ...
          //String mvBackDropPath = TMDB_BASE_URL + W500 + movieInfo_JSONObject.getString(BACKDROP_PATH); // movie poster image thumbnail
            ...
            movieInfo.put(X_MovieInfoEntry.COL_MV_ID, mvId);
            movieInfo.put(X_MovieInfoEntry.COL_TITLE, mvOrgTitle);
            movieInfo.put(X_MovieInfoEntry.COL_RELEASEDATE, mvReleaseDate);
            movieInfo.put(X_MovieInfoEntry.COL_POPULARITY, mvPopularity);
            movieInfo.put(X_MovieInfoEntry.COL_VOTE_AVERAGE, mvVoteAverage);
            movieInfo.put(X_MovieInfoEntry.COL_VOTE_COUNT, mvVoteCount);
            movieInfo.put(X_MovieInfoEntry.COL_FAVOURITES, 0);
            movieInfo.put(X_MovieInfoEntry.COL_OVERVIEW, mvOverview);
            movieInfo.put(X_MovieInfoEntry.COL_POSTERLINK, mvPosterPath);
            movieInfo.put(X_MovieInfoEntry.COL_BACKDROP_PATH, mvBackDropPath);

            //Log.d(LOG_TAG, "---------- get_X_MovieInfoFromJson --------- ");

            //xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
            add_X_MovieInfo(movieInfo, context);
    ...
    private static
    long add_X_MovieInfo(ContentValues movieInfo, Context context){

        long mValueOf_ID = 0;

        Uri uri                 = X_MovieInfoEntry.CONTENT_URI;
        String[] projection     = new String[]{X_MovieInfoEntry._ID};
        String selection        = X_MovieInfoEntry.COL_MV_ID + "=?";
        String[] selectionArg   = new String[]{String.valueOf(movieInfo.getAsLong(X_MovieInfoEntry.COL_MV_ID))};
        ...
        if(mCursor!=null) {
            ...
            if (mCursor.moveToFirst()) {
                // get the '_id' value
                int mColumnIndex = mCursor.getColumnIndex(X_MovieInfoEntry._ID);
                mValueOf_ID = mCursor.getLong(mColumnIndex);
            } else {
                Uri mUri = contentResolver.insert(X_MovieInfoEntry.CONTENT_URI, movieInfo);
