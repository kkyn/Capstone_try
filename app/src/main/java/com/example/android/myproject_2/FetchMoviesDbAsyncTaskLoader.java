 ...
 */
public class FetchMoviesDbAsyncTaskLoader extends AsyncTask<String, Void, Void> {

    private final String LOG_TAG = FetchMoviesDbAsyncTaskLoader.class.getSimpleName();

    ///////////////////////////////////////////////////
    private Context mContext;

    public FetchMoviesDbAsyncTaskLoader(Context context) {
...
//    }
    ...
    //private MoviesSelectedInfo[] getMovieInfoFromJson(String moviesJsonStr) throws JSONException {
        ...
        for (int i = 0; i < resultArray.length(); i++) {
            ...
            long movieId = movieInfoJson.getLong(ID);
            String movieOrgTitle = movieInfoJson.getString(ORIGINAL_TITLE);
            String movieOverview = movieInfoJson.getString(OVERVIEW); // plot synopsis
            String movieVoteAverage = movieInfoJson.getString(VOTE_AVERAGE);  // user rating
            long movieVoteCount = movieInfoJson.getLong(VOTE_COUNT);  // user, sum number of votes
            String moviePopularity = movieInfoJson.getString(POPULARITY);
            String movieReleaseDate = movieInfoJson.getString(RELEASE_DATE);

            String moviePosterPath = TMDB_BASE_URL + W500 + movieInfoJson.getString(POSTER_PATH);
            String movieBackDropPath = TMDB_BASE_URL + W500 + movieInfoJson.getString(BACKDROP_PATH); // movie poster image thumbnail

            myMovSlctdInfo[i] = new MoviesSelectedInfo(
                movieId, movieOrgTitle,
                movieOverview, movieVoteAverage, moviePopularity,
    ...
    private void addPopularTable(MoviesSelectedInfo movieInfo, int arrayIndx, long rowId) {
        ...
        if (mCursor.moveToFirst()) {
            // get the '_id' value
            int mColIndx = mCursor.getColumnIndex(MovieInfoEntry._ID);
            ...
            if (arrayIndx == (int )popularTableRow_Id) {
                ...
                mCValues.put(PopularEntry.COL_MV_ID, movieInfo.mId);
                mCValues.put(PopularEntry.COL_TITLE, movieInfo.mOriginalTitle);
                mCValues.put(PopularEntry.COL_POSTER, movieInfo.mPoster);
                mCValues.put(PopularEntry.COL_KEY_ID, rowId);
    ...
    private long addMovieInfo(MoviesSelectedInfo movieInfo, String sortBy) {
        ...
        if (mCursor.moveToFirst()) {
            ...
            mCValues.put(MovieInfoEntry.COL_MV_ID,          movieInfo.mId);
            mCValues.put(MovieInfoEntry.COL_TITLE,          movieInfo.mOriginalTitle);
            mCValues.put(MovieInfoEntry.COL_RELEASEDATE,    movieInfo.mReleaseDate);
            mCValues.put(MovieInfoEntry.COL_OVERVIEW, movieInfo.mOverview);
            mCValues.put(MovieInfoEntry.COL_POSTERLINK,     movieInfo.mPoster);
            mCValues.put(MovieInfoEntry.COL_VOTE_AVERAGE,   movieInfo.mVoteAverage);
            mCValues.put(MovieInfoEntry.COL_POPULARITY,     movieInfo.mPopularity);
            mCValues.put(MovieInfoEntry.COL_VOTE_COUNT,     movieInfo.mVoteCount);
            /*
            ...
            Uri insertUri = mContext.getContentResolver().insert(MovieInfoEntry.CONTENT_URI, mCValues);
    ...
    private void addRatingTable(MoviesSelectedInfo movieInfo, int arrayIndx, long rowId) {
        ...
        if (mCursor.moveToFirst()) {
            ...
            if (arrayIndx == (int )ratingTableRow_Id) {
                ...
                Log.d(LOG_TAG, "  xxxx addPopularTable  add Content Values");
                ContentValues mCValues = new ContentValues();

                mCValues.put(RatingEntry.COL_MV_ID, movieInfo.mId);
                mCValues.put(RatingEntry.COL_TITLE, movieInfo.mOriginalTitle);
                mCValues.put(RatingEntry.COL_KEY_ID, rowId);
